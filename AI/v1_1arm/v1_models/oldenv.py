...
class JustDoIt_Old(gym.Env):
    def __init__(self, gridDim = 30, holds = np.column_stack(( np.full(100, 15.0),np.linspace(0.0, 30.0, 100, endpoint=False) )), angleChange = 10, energy=500):
        self.gridDim = gridDim
        self.angleChange = angleChange
        target_idx = np.random.randint(0, len(holds))
        self.target_hold = holds[-1] # The last possible hold
        self.holds = holds
    ...
    def step(self, action): # Remember the action is typically sampled on the concept of correct "approximations" until the policy has been refine by the optimal q values, and then the policy pi will behave greedily
        ...
        #print(self.past_distance_deltas_armR)
